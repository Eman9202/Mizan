...
test('voice requests microphone and cleans up on service failure',async()=>{const a=boot();await a.select('#aiVoiceBtn').onclick();assert.equal(a.requests,1);assert.equal(a.fetches,1);assert.equal(a.stopped,1);assert.equal(a.select('#aiVoiceBtn').disabled,false);assert(!a.select('#voiceStage').classList.contains('active'))});
test('voice denial displays permission guidance',async()=>{const a=boot({mediaDenied:true});await a.select('#aiVoiceBtn').onclick();assert.match(a.select('#homeStatus').textContent,/الميكروفون مرفوض/);assert.equal(a.fetches,0)});
test('closing while permission is pending releases the late stream',async()=>{const a=boot({pendingMedia:true});const running=a.select('#aiVoiceBtn').onclick();a.events.click({target:{closest:s=>s==='#closeVoiceStage'}});a.resolveMedia();await running;assert.equal(a.stopped,1);assert.equal(a.fetches,0);assert(!a.select('#voiceStage').classList.contains('active'))});
test('voice timeout re-enables controls and releases late media',async()=>{const a=boot({pendingMedia:true});const running=a.select('#aiVoiceBtn').onclick();[...a.timers.values()].find(x=>x.ms===20000).fn();a.resolveMedia();await running;assert.equal(a.stopped,1);assert.equal(a.select('#aiVoiceBtn').disabled,false);assert.match(a.select('#homeStatus').textContent,/مهلة/)});
...
test('connected voice drives avatar states and closes cleanly',async()=>{const a=boot({connected:true});await a.select('#aiVoiceBtn').onclick();assert.equal(a.fetches,2);assert(a.select('#aiVoiceBtn').classList.contains('live'));a.channel.onmessage({data:JSON.stringify({type:'response.output_audio.delta'})});assert(a.select('#stageAvatar').classList.contains('talking'));a.channel.onmessage({data:JSON.stringify({type:'output_audio_buffer.stopped'})});assert(a.select('#stageAvatar').classList.contains('listening'));a.events.click({target:{closest:s=>s==='#closeVoiceStage'}});assert.equal(a.stopped,1);assert(!a.select('#voiceStage').classList.contains('active'));assert(!a.select('#stageAvatar').classList.contains('listening'))});
